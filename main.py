...
def train_generator_MLE(gen, gen_opt, oracle, real_data_samples, epochs):
    ...
    for epoch in range(epochs):
        ...
        select_samples = random.sample(real_data_samples, POS_NEG_SAMPLES)
...
def train_generator_PG(gen, gen_opt, oracle, dis, num_batches):
    ...
    msg = ' oracle_sample_NLL = %.4f, time = %.2f' % (oracle_loss, time.time()-start_time)
    log.append(msg)
    print(msg)


def train_discriminator(discriminator, dis_opt, pos_samples, neg_samples, generator, oracle, d_steps, epochs):
    """
    ...
    for d_step in range(d_steps):
        ...
        dis_inp, dis_target = helpers.prepare_discriminator_data(pos_samples, s, gpu=CUDA)
        for epoch in range(epochs):
...
if __name__ == '__main__':
    ...
    with open(curtime + "_dis.config", "w") as f:
        f.write("\n".join(log))
        for k in param.keys():
            f.write(k)
            f.write(" = ")
    ...
    train_discriminator(dis, dis_optimizer, pos_samples, neg_samples, gen, oracle, 10, 3)  # 50, 3
    ...
    for epoch in range(ADV_TRAIN_EPOCHS):
        ...
        train_discriminator(dis, dis_optimizer, pos_samples, neg_samples, gen, oracle, 5, 3)
